...
const Services = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-[#0f0f0f] text-gray-100' : 'bg-white text-gray-800'}`}>
      {/* Hero Section */}
      <section className={`relative h-screen flex items-center justify-center bg-gradient-to-br ${darkMode ? 'from-black via-[#0f0f0f] to-primary/20' : 'from-white via-transparent'} overflow-hidden`}>
        ...
        >
          <div className="space-y-8">
            <h1 className="text-6xl md:text-9xl font-bold font-clashdisplay bg-gradient-to-r from-primary via-cyan-400 to-emerald-400 bg-clip-text text-transparent leading-[0.85]">
              ...
              <motion.span
                className={`inline-block bg-gradient-to-r bg-clip-text text-transparent${darkMode ? 'from-gray-200 to-gray-400' : 'from-gray-400 to-gray-800'}`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
                Tech Solutions
            ...
            >
              ...
              <Link
                to="/contact"
                className={`relative z-10 inline-block px-12 py-6 bg-gradient-to-br ${darkMode ? 'from-primary to-cyan-400' : 'from-primary to-cyan-400'} text-dark-primary rounded-2xl hover:shadow-2xl text-lg font-bold`}
              >
      ...
      <section className={`relative py-32 ${darkMode ? 'bg-gradient-to-b from-[#0a0a0a] to-[#111111]' : 'bg-gradient-to-b from-white to-gray-200'}`}>
        <div className="max-w-screen-2xl mx-auto px-4 grid md:grid-cols-3 gap-8">
          {values.map((value, index) => (
            <motion.div
              ...
              className={`p-8 ${darkMode ? 'bg-gradient-to-b from-[#151515] to-[#0f0f0f]' : 'bg-white'} rounded-3xl border border-gray-800/50 hover:border-cyan-400/30 transition-all`}
            >
      ...
      {/* Services Grid */}
      <section className={`relative py-32 ${darkMode ? 'bg-gradient-to-b from-[#111111] to-[#0a0a0a]' : 'bg-gradient-to-b from-white to-gray-200'}`}>
        <div className="max-w-screen-2xl mx-auto px-4">
          <motion.h2
            className={`text-4xl md:text-6xl font-bold text-center mb-20 ${darkMode ? 'bg-gradient-to-r bg-clip-text from-primary to-cyan-500' : 'text-gray-800'}`}
            initial={{ opacity: 0 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div
                ...
                className={`group relative p-8 ${darkMode ? 'bg-gradient-to-b from-[#151515] to-[#0f0f0f]' : ''} rounded-3xl border border-gray-800/50 hover:border-cyan-400/30 transition-all`}
              >
                ...
                <div className="relative z-10">
                  <motion.div
                    className={`text-4xl mb-6 ${darkMode ? 'bg-gradient-to-r text-primary ' : 'text-blue-600'}`}
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {service.tech.map((tech, i) => (
                      <motion.span
                        key={i}
                        className={`px-3 py-1.5 ${darkMode ? 'bg-[#1a1a1a] text-gray-300' : 'bg-white text-black'} border border-gray-800 rounded-full text-sm  hover:scale cursor-default`}
                        whileHover={{ scale: 1.05 }}
                      >
                        {tech}
                      </motion.span>
          ...
          >
            {['React', 'Node.js', 'AWS', 'Python', 'TensorFlow', 'Docker', 'Kubernetes', 'AI/ML'].map((tech, index) => (
              <motion.div
                key={index}
                whileHover={{ y: -5 }}
                className={`p-6 ${darkMode ? 'bg-gradient-to-b from-[#151515] to-[#0f0f0f]' : 'bg-white'} rounded-2xl border border-gray-800/50 text-center hover:border-cyan-400/30 transition-all`}
              >
        ...
        <div className="max-w-screen-xl mx-auto px-4 text-center relative z-10">
          ...
          >
            <div className="space-y-6">
              <h2 className={`text-5xl md:text-7xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
